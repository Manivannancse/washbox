...
class Wash_Model extends CI_Model
{
	...
	function placeOrder($userId) 
	{
		...
		$preorderId= explode("-", $userdata[0]['order_id']);
		
		if($preorderId[0]!=$DateFomt){
			$Fomtid=$DateFomt."-1";	
		}else{
	...
	function pickTiming($date){
		...
		$mydate[0]=ltrim($mydate[0], '0');
		$mydate[1]=ltrim($mydate[1], '0');
		$mydate=implode("-",$mydate);
		//echo $mydate;
		//echo $date;
		date_default_timezone_set('Asia/Kolkata');
		$possibleDelviery = date('ga',strtotime("+4 hours"));
		...
		else
		{
			if($date==$mydate)
			{
				...
				{
					$time = date('ga',strtotime("+3 hours"));
					if(strtotime($timeSplit[0]) >= strtotime($time) || strtotime($timeSplit[1]) > strtotime($time)){
						array_push($data,array("name" =>$row ,"disable" => "false"));
					}else{
						array_push($data,array("name" =>$row ,"disable" => "true"));
					}
	...
	return $this->db->query($select)->result_array();
       ...
       function password_token($id,$email){
		...
		if($randnum!=""){
		 ...
		 $this->email->set_newline("\r\n");
                 $subject=$this->email->subject('Welcome To washBox');
		 $body = $this->load->view('forgotpasswordTemplate',$data,TRUE);
 		 $this->emailsending($email,$body,$subject);
